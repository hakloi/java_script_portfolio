...
function preload() {
    ...
    Jap = loadFont('fonts/HIRAGANA.ttf');
    img = loadImage('images/magic.png');
    end2 = loadImage('images/end2.jpg');
    end3 = loadImage('images/end3.jpg');
    ...
    panic = loadSound('sounds/male-heavy-breathing-whimper_f1infdnd.mp3');
    pistol = loadSound('sounds/16551_1460656891.mp3');
    cry = loadSound('sounds/cry.mp3');
    humming = loadSound('sounds/d432b287428f1f4.mp3');
...
function startGame(){
    ...
    */
    ...
    mus.isBreathPlaying = false;
    mus.isCryPlaying = false;
}
...
function draw(){
    if (!vars.isStateControllerEnabled) {
        return; 
    }
    ...
    */
    ...
    if (!vars.isStateControllerEnabled) {
        if(num == 1){ //good 1
            fill(0, 99);
          rect(vars.end_x - 350, 0, width, height);
          rect(vars.end_x - 350, 0, width, height);
          rect(vars.end_x - 350, 0, width, height);
          image(end2, vars.end_x - 338, 0);
          fill(0, 99);
          rect(vars.end_x - 350, height / 1.5, width, height);
          textFont(Uni, 'bold');
          textSize(30);
          textStyle(BOLD);
          fill(255);
          text("Когда я проснулся, мне показалось, словно вдохновение окутало меня с ног до\nголовы. Я взялся за ручку и долго, не отрываясь, писал свои мысли. Я словно\nслучайно иногда вспоминал отрывки своего сна - какая-то ведьма, птицы,\nловушки. Но все равно в этом сне было что-то судьбоносное. Так мне казалось.", vars.end_x - 300, height / 1.35)
            humming.play();
        } else if(num ==2) { //bad 2
            fill(0, 99);
              rect(vars.end_x - 350, 0, width, height);
              ...
              textStyle(BOLD);
              fill(255);
              text("Когда я проснулся тело изнемогало от усталости, холода и сырости. Пустые\nтетради лежали повсюду, напоминая о моих неудачах. Отчаяние, очередной отказ\nот издательства, раздражающий запах твоих духов - все это причины,\nпочему я сошел с ума. Я направил пистолет в рот и положил палец на курок.", vars.end_x - 300, height / 1.35)
              panic.play();
              isPanicPlayed = true;
              if (isPanicPlayed) {
                setTimeout(function() {
...
function checkFlagpole(flagpole) {
    ...
    if (dist(character.CharX, character.CharY, flagpole.pos_X+35, 432) < 50) {
        ...
        if(flagpole.ending =='good' && vars.game_score == collectables.length){
            textFont(Uni, 'bold');
            textSize(50);
            text("GOOD ENDING!\nPress space to wake up", vars.note_x+180, height/2);
            character.isRight = false;
            character.isLeft = false;
            keyPressed =false; 
            if (keyCode == 32 || keyCode === UP_ARROW) { 
                vars.isStateControllerEnabled = false;
                num =1;
            }
...
function stateController(){
    ...
    if (character.CharX >= 575 && character.CharX <= 580 && !mus.isBreathPlaying) {
        breath.play();
        mus.isBreathPlaying = true;
    }
    
    if (character.CharX >= 1900 && character.CharX <= 1910 && !mus.isCryPlaying) {
        cry.play();
        mus.isCryPlaying = true;
    }
}
